...
router.get('/', async (req, res) => {
	...
	const SelectedTeamBox =
		`
		SELECT
	leagueID, pt.ptID, teamFullName, leagueRank, matchCount, matchWinCount, matchDefeatCount, matchWinRate, gameWinCount, gameDefeatCount, imgPath
FROM # leagueRank
	(SELECT
		leagueID, ptID, 
		(CASE 
...
INNER JOIN (SELECT ptID, teamFullName FROM participating_teams) AS pt # To Join teamFullName 
	ON leagueRank.ptID = pt.ptID
INNER JOIN (SELECT ptID, imgPath FROM teamIMGpath) AS tip
	ON leagueRank.ptID = tip.ptID
WHERE pt.ptID = '${ptID}'; -- 파라미터 값
	`;
