...
router.get('/', async (req, res) => {
	...
	if (typeof (ptID) == 'string') {
		...
		# 0613 ver
SELECT
	team1 AS ptID, SUBSTRING_INDEX(team1, '-', -1) AS teamABBR ,
	GROUP_CONCAT(WRBlue.minutes) AS yAxis,
	...
	(CASE
		WHEN COUNT(WRBlue.minutes) = 4 THEN GROUP_CONCAT(gameCount_Blue)
		ELSE CONCAT('null, ','null, ','null, ', GROUP_CONCAT(gameCount_Blue))
	END) AS gameCount_Blue,
	(CASE
		WHEN COUNT(WRBlue.minutes) = 4 THEN GROUP_CONCAT(winCount_Blue)
		ELSE CONCAT('null, ','null, ','null, ', GROUP_CONCAT(winCount_Blue))
	END) AS winCount_Blue,
	(CASE
		WHEN COUNT(WRBlue.minutes) = 4 THEN GROUP_CONCAT(ROUND((winCount_Blue / gameCount_Blue) * 100, 1))
		ELSE CONCAT('null, ','null, ','null, ', GROUP_CONCAT(ROUND((winCount_Blue / gameCount_Blue) * 100, 1)))
	END) AS WR_Blue,	
	-- GROUP_CONCAT(gameCount_Red) AS gameCount_Red, GROUP_CONCAT(winCount_Red) AS winCount_Red, GROUP_CONCAT(ROUND((winCount_Red / gameCount_Red) * 100, 1)) AS WR_Red
		(CASE
		WHEN COUNT(WRBlue.minutes) = 4 THEN GROUP_CONCAT(gameCount_Red)
		ELSE CONCAT('null, ','null, ','null, ', GROUP_CONCAT(gameCount_Red))
	END) AS gameCount_Red,
	(CASE
		WHEN COUNT(WRBlue.minutes) = 4 THEN GROUP_CONCAT(winCount_Red)
		ELSE CONCAT('null, ','null, ','null, ', GROUP_CONCAT(winCount_Red))
	END) AS winCount_Red,
	(CASE
		WHEN COUNT(WRBlue.minutes) = 4 THEN GROUP_CONCAT(ROUND((winCount_Red / gameCount_Red) * 100, 1))
		ELSE CONCAT('null, ','null, ','null, ', GROUP_CONCAT(ROUND((winCount_Red / gameCount_Red) * 100, 1)))
	END) AS WR_Red
FROM
	(SELECT
